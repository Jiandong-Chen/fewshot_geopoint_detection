...
def build_map_mask(map_name, mask_root, data_root):
    mask_file = os.path.join(mask_root, map_name + '_expected_crop_region.tif')
    if os.path.exists(mask_file):
        map_mask = Image.open(mask_file)
        map_mask = np.array(map_mask) == 0
    else:
        print('Cannot find an existing mask: ', mask_file, '\n')
        map_mask = cropping_worker(None, map_name + '.json', data_root, '/Users/dong_dong_dong/Downloads/Darpa/train/', True)
